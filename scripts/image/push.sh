...
_ensure_manifest() {
  ...
  for repo in ${IMAGE_REPOS}; do
    image_name="$(_get_image_name "${repo}" "${os}" "${arch}")"
    manifest_name="$(_get_image_manifest_name "${repo}")"
...
_push_image() {
  ...
  for repo in ${IMAGE_REPOS}; do
    docker push "$(_get_image_name "${repo}" "${os}" "${arch}")"
  done

  _ensure_manifest "${os}" "${arch}" || true

  for repo in ${IMAGE_REPOS}; do
    docker manifest push "$(_get_image_manifest_name "${repo}")" || true
  done
}
